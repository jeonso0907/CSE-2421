...
/* Print out the header of the each delta time reulst */
...
/* Draw the graphical output of the firework */
...
/* Print out the text output of the firework */
...
/* Output the data based on the mode */
...
/* Initiate the normal output by setting initial circumstances based on each mode */
...
/* Finalize th output basd on each mode */
...
/* Delete the firework node while printing the final effect */
void effects(struct Sim *site) {

	deleteSome(&site->p2headptr, is_finished, finish, TEXT);
}

/* Fase the firework in graphical mode */
...
/* Print out the rengae safety message for the impact */
...
/* Print the effects based on the given code information */
